...
@Component({
  selector: 'app-add-playlists-modal',
  templateUrl: './add-playlists-modal.page.html',
  styleUrls: ['./add-playlists-modal.page.scss'],
})
export class AddPlaylistsModalPage {
  items = [
  ...
  selectAll() {
    // Logic to handle selecting all options
    this.items.forEach(item => item.isSelected = this.selectAllCheckbox);
    this.checkDoneButton();
  }

  checkDoneButton() {
    // Logic to check if at least one item is selected
    // This will be used to show/hide the "Done" button
  }
